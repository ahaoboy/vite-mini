...
socket.addEventListener('message', async ({data}) => {
  ...
  switch (type) {
    ...
    case 'vue-reload':
      ...
      break;
    case 'vue-rerender':
      console.log('vue-rerender', __VUE_HMR_RUNTIME__)
      console.log('templatePath', `${path}?type=template`)
      const templatePath = `${path}?type=template`;
      console.log('`${templatePath}&t=${timestamp}`', `${templatePath}&t=${timestamp}`)
      // await bustSwCache(templatePath);
      import(`${templatePath}&t=${timestamp}`).then((m) => {
      // import(`aaa`).then((m) => {
        console.log('__VUE_HMR_RUNTIME__', __VUE_HMR_RUNTIME__, path, m)

...
const socket = new WebSocket(`${socketProtocol}://localhost:3001`);
...
socket.addEventListener('message', async ({data}) => {
  const {type, path, changeSrcPath, id, timestamp, customData} = JSON.parse(data);
  if (changeSrcPath) {
  ...
  switch (type) {
    ...
    case 'vue-rerender':
      ...
      import(`${templatePath}&t=${timestamp}`).then((m) => {
        console.log(`[vite] ${path} template updated.`, path, m);
...
//   setInterval(() => {
//     new WebSocket(`${socketProtocol}://localhost:3001`).addEventListener('open', () => {
//       location.reload();
//     });
//   }, 1000);
// });
...
async function updateModule(id, changedPath, timestamp) {
  console.log('updateModule', id,)
  const mod = hotModulesMap.get(id);
  if (!mod) {
    console.error(`[vite] got js update notification for "${id}" but no client callback ` +
...
export const createHotContext = (id) => {
  console.log('===createHotContext', id)
  if (!dataMap.has(id)) {
    dataMap.set(id, {});
  }
